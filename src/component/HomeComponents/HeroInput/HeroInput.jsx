...
const HeroInput = ( { setCars } ) => {
    ...
    const handleSearch = () => {
        if (!textInput){
            ...
            setTextInput("")
            navigate("/search/name/"+textInput);
        }
        
    };

    const handleSearchByPress = (e) => {
        if(e.key === 'Enter') {
            if (!textInput){
                navigate("/")
            }else{
    ...
    return (
        <div className="bg-white rounded-[10px] min-[671px]:rounded-[80px] max-[670px]:p-[20px] p-[10px] min-[671px]:ps-[55px] w-[95%] md:w-[700px] lg:w-[930px] flex items-center gap-[31px] max-[670px]:flex-col max-[670px]:w-fit max-[600px]:gap-5">
            ...
            <input type="text" onKeyDown={(e) => handleSearchByPress(e)} value={textInput} className={`bg-white border-none outline-none min-[671px]:w-full`} dir={language === 'AR' ? 'rtl': 'ltr'} placeholder={placeHolder} onChange={(e) => setTextInput(e.target.value)} />
